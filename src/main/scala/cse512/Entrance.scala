...
object Entrance extends App {
  ...
  override def main(args: Array[String]) {
    val spark = SparkSession
      .builder()
      .appName("CSE512-HotspotAnalysis-YAKUZA") // YOU NEED TO CHANGE YOUR GROUP NAME
      .config("spark.some.config.option", "some-value")//.master("local[*]")
      .getOrCreate()

    //println("answer is " + HotzoneAnalysis.runHotZoneAnalysis(spark, "src/resources/point_hotzone.csv", "src/resources/zone-hotzone.csv").show())
    //println("answer is " + HotcellAnalysis.runHotcellAnalysis(spark, "src/resources/point_hotzone.csv").show())
